...
const prcopt_t prcopt_default = {
    ...
    0,
    1,
    0, /* rcvstds,armaxiter,estion,esttrop,dynamics,tidecorr */
    1,
    0,
    0,
    0,
    ...
    0.05,
    0.1,
    0.01, /* elmaskar,elmaskhold,thresslip,varholdamb,gainholdamb */
    30.0,
    1000.0,
...
static char *obscodes[] = {
    /* observation code strings revised by Lye */
...
static char codepris[7][MAXFREQ][16] = {
    ...
    /* 1:L1/E1/B1C, 2:L2/B1I, 3:L5/E5a/L3, 4:E6/LEX/B3/G2a, 5:B2I/E5b, 6:E5(a+b)/, 7:S  8:G1a */
    /*    0         1          2          3         4         5     */
    {"PCYWMNSL", "PYWCMNDLSX", "IQX", "", "", "", ""},
    /* GPS */                                            /* GPS CPYWMNSL, PYWCMNDSLX*/
    {"CPABX", "PCABX", "IQX", "", "", "", ""},           /* GLO */
 ...
 *-----------------------------------------------------------------------------*/
extern int satexclude(int sat, double var, int svh, const prcopt_t *opt, gtime_t time)
{
    ...
    {
        result = is_NLOS(id, week, gpst);
        // trace(2, "is_NLOS(%s, %d, %d) -> %d\n", id, week, gpst, result);
        if (result > 0)
        {
            trace(3, "%d %d excludes sat: %s %d\n", week, gpst, id, result);
            return 1;
        }
    }

    if (lzw_config.vs_val == 1)
    {
        result = is_VS(id, week, gpst);
        // trace(2, "is_VS(%s, %d, %d) -> %d\n", id, week, gpst, result);
        if (!result)
        {
            trace(3, "%d %d %s not VS in RTK\n", week, gpst, id);
            return 1;
        }
    }
                   ...
                   const snrmask_t *mask)
{
    ...
    if (!mask->ena[base] || freq < 0 || freq >= NFREQ || snr == 0)
        return 0;
...
extern unsigned int getbituMod(const unsigned char *buff, int pos, int len, int scale)
{
    ...
    int i;
    for (i = pos; i < pos + len; i++)
        bits = ((bits << 1) + ((buff[i / 8] >> (7 - i % 8)) & 1u)) * (scale);
    return bits;
}
                   ...
                   double *xp, double *Pp)
{
    ...
    if (!(info = matinv(Q, m)))
    {
        ...
        matmul("NN", n, n, n, 0.0, E, A, 1.0, Pp);*/
        /*this addition is not right*/
    }
#if 0
                        ...
                        const pcvs_t *pcvs)
{
    ...
    trace(5, "searchpcv: sat=%2d type=%s\n", sat, type);
...
extern void traceobs(int level, const obsd_t *obs, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        fprintf(fp_trace, "obs_export (%2d) %s %-3s rcv%d %13.3f %13.3f %13.3f %13.3f %13.3f %13.3f\n", i + 1, str, id, obs[i].rcv, obs[i].L[0], obs[i].L[1], obs[i].P[0],
                    obs[i].P[1], obs[i].D[0], obs[i].D[1]);
    }
    fflush(fp_trace);
}
...
extern double satwavelen(int sat, int frq, const nav_t *nav)
{
    ...
    {
        if (frq == 0)
            return CLIGHT / FREQL1; /* E1 */
        else if (frq == 1)
            return CLIGHT / FREQE5b; /* E5b */
        else if (frq == 2)
            return CLIGHT / FREQL5; /* E5a */
        else if (frq == 3)
            return CLIGHT / FREQE6; /* E6 */
        else if (frq == 5)
            return CLIGHT / FREQE5ab; /* E5ab */
    }
    else
    { /* GPS,QZS */
        if (frq == 0)
            return CLIGHT / FREQL1; /* L1 */
        else if (frq == 1)
            return CLIGHT / FREQL2; /* L2 */
        else if (frq == 2)
            return CLIGHT / FREQL5; /* L5 */
        else if (frq == 3)
            return CLIGHT / FREQE6; /* L6/LEX */
        else if (frq == 6)
            return CLIGHT / FREQs; /* S */
    }
    return 0.0;
}
/* geometric distance ----------------------------------------------------------
                 ...
                 double *gmfh, double *gmfw)
{
    ...
    for (i = 0; i <= n; i++)
    {
        for (j = 0; j <= MIN(i, m); j++)
        {
            ...
            sum1 = 0.0;
            for (k = 0; k <= ir; k++)
                sum1 += pow(-1, k) * dfac[2 * i - 2 * k] / dfac[k] / dfac[i - k] / dfac[i - j - 2 * k] * pow(t, (i - j - 2 * k));
...
extern int rtk_uncompress(const char *file, char *uncfile)
{
    ...
    else if ((p = strrchr(tmpfile, '.')) && strlen(p) > 3 && (*(p + 3) == 'd' || *(p + 3) == 'D' || strstr(p, ".crx") || strstr(p, ".CRX")))
    {
