...
#define WEIGHTOPT_CNR_EL_DEL 2 /* weighting option: cnr-elevation-dependent */
...
{                                           /* time struct */
    ...
    /* lzw global variables*/
    extern PyObject *pFunc_checksat, *pFunc_storeinfo, *pFunc_save, *pFunc_getval, *pFunc_checkvs;
    extern struct LZW_CONFIG{
        char *nlos_env, *varr_env, *vs_env, *k_env, *ar_env;
        int nlos_val, varr_val, vs_val, ar_val;
        double k_val;
    } lzw_config;

    /* lzw functions*/
    EXPORT double get_coff(char *id, int week, int gpst);
    EXPORT int is_NLOS(char *id, int week, int gpst);
    EXPORT int is_VS(char *id, int week, int gpst);
    EXPORT int store_info(gtime_t gtime, char *key, char *val);
    ...
    EXPORT int satexclude(int sat, double var, int svh, const prcopt_t *opt, gtime_t time);
