...
static double varerr(const prcopt_t *opt, double el, double snr_rover, int sys, double k_coff)
{
    double a, b, snr_max;
...
static double varerr1(const prcopt_t *opt, double el, int sys, double coff, const obsd_t *obs) // 传统模式上修改
{
    double fact = 1.0, varr;
    double cn0 = obs->SNR[0] * 0.25;
    double ssi_tmp = cn0 / 5.0;
    double ssi = ssi_tmp < 9 ? ssi_tmp : 9.0;
    double a, b;
    trace(4, "pntpos varerr1 coff=%f\n", coff);
                     ...
                     int iter, const prcopt_t *opt, double *var)
{
    ...
    *var = 0.0;
    ...
    if (NFREQ >= 3 && (sys & (SYS_GAL | SYS_SBS | SYS_BDS)))
        j = 2;
                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    for (i = *ns = 0; i < n && i < MAXOBS; i++)
    {
        ...
        if ((P = prange(obs + i, nav, azel + i * 2, iter, opt, &vmeas)) == 0.0){
            trace(4, "sat=%d prange error\n", obs[i].sat);
            continue;
        }

        /* excluded satellite? */
        if (satexclude(obs[i].sat, vare[i], svh[i], opt, obs[i].time)){
            trace(4, "sat=%2d excluded satellite\n", obs[i].sat);
            continue;
        }
        ...
        {
            trace(2, "sat=%2d tropcorr error\n", obs[i].sat);
            continue;
        }
        /* pseudorange residual */
        v[nv] = P - (r + dtr - CLIGHT * dts[i * 2] + dion + dtrp);
        satno2id(obs[i].sat, id);
        trace(4, "%s P=%f r=%f dtr=%f dts=%f dion=%f dtrp=%f resp=%f\n", id, P, r, dtr, CLIGHT * dts[i * 2], dion, dtrp, v[nv]);
        ...
        /* error variance */
        gpst = (int)time2gpst(obs[i].time, &week);
        double coff_val = lzw_config.nlos_val >= 2 ? get_coff(id, week, gpst) : 1;
        if (lzw_config.varr_val == 0)
            var[nv++] = varerr(opt, azel[1 + i * 2], snr_rover, sys, coff_val) + vare[i] + vmeas + vion + vtrp;
        else
        {
            var[nv++] = varerr1(opt, azel[1 + i * 2], sys, coff_val, obs + i) + vare[i] + vmeas + vion + vtrp;
        }

        // trace(3, "sat=%2d(%s) azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n", obs[i].sat, id,
        //       azel[i * 2] * R2D, azel[1 + i * 2] * R2D, resp[i], sqrt(var[nv - 1]));
        trace(4, "sat=%2d(%s) azel=%5.1f %4.1f res=%7.3f sig=%5.3f dion=%.3f dtrp=%.3f dts=%.3f \n", obs[i].sat, id,
              azel[i * 2] * R2D, azel[1 + i * 2] * R2D, resp[i], sqrt(var[nv - 1]), dion, dtrp, dts[i * 2] * CLIGHT);

        trace(5, "rpd_export %s %s %f\n", time_str(obs[i].time, 3), id, resp[i] + r);
    }
...
/*     for (i = 0; i < ns; i++)
    {
        trace(2, "valsol  : azel[%d] = %f %f\n", i, azels[i * 2] * R2D, azels[1 + i * 2] * R2D);
    } */
    trace(4, "valsol azels : ns=%d\n", ns);
                  ...
                  int *vsat, double *resp, char *msg)
{
    ...
    trace(3, "estpos  : n=%d init pos=%.3f %.3f %.3f\n", n, x[0], x[1], x[2]);
        

    for (i = 0; i < MAXITR; i++)
    {
        trace(3, "estpos  : n=%d iter=%d\n", n, i);
        /* pseudorange residuals */
        ...
        if (norm(dx, NX) < 1E-4)
        {
            ...
            if (opt->mode >= PMODE_SINGLE){
                // storeage
                char msg[16];
                sprintf(msg, "%2d", sol->ns);
                store_info(obs[0].time, "ns_spp", msg);
            }

            /* validate solution */
            if ((stat = valsol(azel, vsat, n, opt, v, nv, NX, msg, obs[0].time)))
            {
                  ...
                  char *msg)
{
    ...
    // {
        for (i = 0; i < n; i++)
        {
            if (vsat[i])
            {
                // gpst = (int)time2gpst(obs[i].time, &week);
                satno2id(obs[i].sat, id);
                store_info(obs[0].time, "VS_spp", id);
            }
        }
    // }
    ...
    if (ssat)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            // append rs to ssat
            for (int j = 0; j < 3; j++)
                ssat[obs[i].sat - 1].rs[j] = rs[j + i * 6];
            // dts
            ssat[obs[i].sat - 1].dts[0] = dts[i * 2];
            ssat[obs[i].sat - 1].dts[1] = dts[1 + i * 2];
            // dtr
            ssat[obs[i].sat - 1].dtr = sol->dtr[0];
        }
    }
    free(rs);
    free(dts);
