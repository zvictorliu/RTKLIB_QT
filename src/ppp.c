...
static double varerr(int sat, int sys, double el, double snr_rover, int freq,
                     int type, const prcopt_t *opt, double k_coff)
{
    double a, b, snr_max;
    double fact = 1.0;
    double sinel = sin(el);
                      ...
                      const prcopt_t *opt, double coff, const obsd_t *obs)
{
    ...
    fact *= opt->ionoopt == IONOOPT_IFLC ? 3.0 : 1.0; // iono-free
                      ...
                      double *Lc, double *Pc)
{
    ...
    *Lc = *Pc = 0.0;
                              ...
                              double *Lc, double *Pc, osrd_t *osr)
    {
        ...
        for (i = 0; i < NFREQ; i++)
        {
            ...
            if (obs->P[i])
                P[i] = obs->P[i] - dants[i] - dantr[i];
            if (i==0)
                trace(5, "    sat=%2d dants=%6.3f dantr=%6.3f phw=%6.3f\n", obs->sat, dants[i], dantr[i], phw);
            ...
            // if (opt->ionoopt == IONOOPT_EST && nav->bia)
            {
                ...
                else if (nav->bia) // OSB
                {
                    if (nav->bia[obs->sat - 1].cBias[obs->code[i] - 1] != 0)
                    {
                        ...
                        //               {
                        //               case SYS_BDS:
                        ///*triple frequency DCB correction when the clock is reference to P1/P3 IF combination, correct P1P2P3 BY XIANG C3=1/(1-gamma5)*/
                        //                   DCB12 = nav->bia[obs->sat - 1].cBias[CODE_L2I - 1] - nav->bia[obs->sat - 1].cBias[CODE_L7I - 1];/*B2b 1207.14*/
                        //                   DCB13 = nav->bia[obs->sat - 1].cBias[CODE_L2I - 1] - nav->bia[obs->sat - 1].cBias[CODE_L6I - 1];/*B3 1268.52*/
                        //
                        //                   C1 = SQR(lam[2]) / (SQR(lam[2]) - SQR(lam[0]));/*C1 = gamma1 / (1 - gamma3) */
                        //                   C3 = SQR(lam[0]) / (SQR(lam[2]) - SQR(lam[0]));/*C3 = 1 / (1 - gamma3) */
                        //                   C2 = SQR(lam[0]) / (SQR(lam[1]) - SQR(lam[0]));/*C2 = 1 / (1 - gamma2) */

                        //                   if (P[0] != 0) 	P[0] += (C3 * DCB13);
                        //                   if (P[1] != 0) P[1] -= (  -DCB12+ C3/C2 * DCB13);
                        //                   if (P[2] != 0) 	P[2] += (C1 * DCB13);/*-(C2/C3*DCB12-DCB15);*/
                        //                   CAC

                        //               default:
                        //                   P[i] -= nav->bia[obs->sat - 1].cBias[obs->code[i] - 1];/*aligh to IF clock using ABS BIA products*/
                        //                   L[i] -= nav->bia[obs->sat - 1].lBias[obs->code[i] - 1];/*phase aligh to IF clock using ABS BIA products*/
                        //               }
                        P[i] -= nav->bia[obs->sat - 1].cBias[obs->code[i] - 1]; /*aligh to IF clock using ABS BIA products*/
                        trace(5, "OSB sat=%2d bia= %f ns\n", obs->sat, nav->bia[obs->sat - 1].cBias[obs->code[i] - 1] / (1E-9 * CLIGHT));
                        // 到底是加还是减
                        // L[i] -= nav->bia[obs->sat - 1].lBias[obs->code[i] - 1];/*phase aligh to IF clock using ABS BIA products*/
                ...
                else // DCB
                {
                    ...
                    {
                        P[i] += nav->cbias[obs->sat - 1][1];
                        trace(3, "    P1-C1 sat=%2d bia= %f ns\n", obs->sat, nav->cbias[obs->sat - 1][1] / (1E-9 * CLIGHT));

                        // P1-P2 dcb correction
                        P[0] += nav->cbias[obs->sat - 1][0] / (SQR(lam[1]) / SQR(lam[0]) - 1);
                        trace(3, "    P1-P2 sat=%2d bia= %f ns\n", obs->sat, nav->cbias[obs->sat - 1][0] / (1E-9 * CLIGHT));
                    }
                    else if (obs->code[i] == CODE_L2C || obs->code[i] == CODE_L2X ||
                             obs->code[i] == CODE_L2L || obs->code[i] == CODE_L2S)
                    {
        ...
        for (f = 0; f < NF(&rtk->opt); f++)
        {
            ...
            for (i = 0; i < MAXSAT; i++)
            {
                    ...
                    rtk->opt.modear == ARMODE_INST || clk_jump)
                {
                    ...
                    // trace(3, "reset ppp phase bias: sat=%2d f=%d\n", i + 1, f + 1);
                }
            }
            for (i = k = 0; i < n && i < MAXOBS; i++)
            {
                ...
                else if (L[f] != 0.0 && P[f] != 0.0)
                {
                    ...
                    // trace(2, "ion computed from P[0] and P[%d] = %f\n", l, ion);
            ...
            for (i = 0; i < n && i < MAXOBS; i++)
            {
                ...
                trace(4, "    sat=%2d P[%d]=%f\n", sat, j, rtk->P[j + j * rtk->nx]);
                ...
                trace(4, "    sat=%2d bias=%.3f P[%d]=%f\n", sat, bias[i], j, rtk->P[j + j * rtk->nx]);
            }
        }
    }
                       ...
                       double *azel, osrd_t *osr)
    {
        ...
        for (i = 0; i < n && i < MAXOBS; i++)
        {
            ...
            if (!(sys = satsys(sat, NULL)) || !rtk->ssat[sat - 1].vs ||
                satexclude(obs[i].sat, var_rs[i], svh[i], opt, obs[i].time) || exc[i]) /* excluded satellite */
            {
                exc[i] = 1;
                continue;
            }
        ...
        /*}*/
                ...
                { /* phase bias */
                    if ((bias = x[IB(sat, j / 2, opt)]) == 0.0){
                        // trace(2, "no phase bias for sat=%2d x[%d]=%f\n", sat, IB(sat, j / 2, opt), x[IB(sat, j / 2, opt)]);
                        continue;
                    }
                ...
                // 为什么 dion 是乘以C的？单频时只有+-1
                trace(5, "    dts=%.4f ion=%.1f * %.4f dcb1=%.4f dcb2=%.4f bias=%.4f\n", CLIGHT * dts[i * 2], C , dion, dcb1, dcb2, bias);
                // trace(3, "  dtr=%.4f dts=%.4f ion=%.4f dcb1=%.4f dcb2=%.4f bias=%.4f\n", dtrp, CLIGHT * dts[i * 2], C * dion, dcb1, dcb2, bias);
                ...
                if (lzw_config.varr_val == 0)
                    var[nv] = varerr(obs[i].sat, sys, azel[1 + i * 2], 0.25 * rtk->ssat[sat - 1].snr_rover[j / 2], j / 2, j % 2, opt, coff_val); /* +
                           vart+SQR(C)*vari+var_rs[i];*/
                else{
                    var[nv] = varerr1(obs[i].sat, sys, azel[1 + i * 2], j / 2, j % 2, opt, coff_val, obs+i);
                }
                if (sys == SYS_GLO && j % 2 == 1)
                    var[nv] += VAR_GLO_IFB;

                trace(3, "    %s sat=%2d %s %s%d res=%9.4f sig=%9.4f el=%4.1f y=%.4f\n", str, sat, id,
                      j % 2 ? "P" : "L", j / 2 + 1, v[nv], sqrt(var[nv]), azel[1 + i * 2] * R2D, y);
                ...
                {
                    trace(2, "    outlier rejected (pre) %s sat=%2d %3s %s%d res=%9.4f el=%4.1f\n",
                        str, sat, id, j % 2 ? "P" : "L", j / 2 + 1, v[nv], azel[1 + i * 2] * R2D);
                    exc[i] = 1;
                ...
                {
                    trace(3, "    large post-fit residual: sat=%2d %3s %s%d res=%9.4f sig=%9.4f el=%4.1f\n",
                          sat, id, j % 2 ? "P" : "L", j / 2 + 1, v[nv], sqrt(var[nv]), azel[1 + i * 2] * R2D);
                    obsi[ne] = i;
                    frqi[ne] = j;
                    ve[ne] = v[nv];
                    ne++;
        ...
        if (post && ne > 0)
        {
            ...
            sat = obs[maxobs].sat;
            satno2id(sat, id);
            trace(2, "    outlier rejected (post) %s sat=%2d %3s %s%d res=%9.4f el=%4.1f\n",
                  str, sat, id, maxfrq % 2 ? "P" : "L", maxfrq / 2 + 1, vmax, azel[1 + maxobs * 2] * R2D);
            exc[maxobs] = 1;
    ...
    extern void pppos(rtk_t * rtk, const obsd_t *obs, int n, const nav_t *nav)
    {
        ...
        /* exclude measurements of eclipsing satellite (block IIA) 踢星*/
        if (rtk->opt.posopt[3])
        {
        ...
        for (i = 0; i < MAX_ITER; i++)
        {
            ...
            /* postfit residuals 后验残差*/
            if (ppp_res(i + 1, obs, n, rs, dts, var, svh, dr, exc, nav, xp, rtk, v, H, R, azel, osr))
            {
                matcpy(rtk->x, xp, rtk->nx, 1);
                matcpy(rtk->P, Pp, rtk->nx, rtk->nx);
                trace(2, "%s ppp float rr= %f %f %f\n", str, rtk->x[0], rtk->x[1], rtk->x[2]);
                stat = SOLQ_PPP;
                break;
            }
        }
        ...
        if (stat == SOLQ_PPP)
        {
                ...
                ppp_res(9, obs, n, rs, dts, var, svh, dr, exc, nav, xp, rtk, v, H, R, azel, osr))
            {
                ...
                if (norm(std, 3) < MAX_STD_FIX){
                    stat = SOLQ_FIX;
                    trace(2, "%s ppp fix rr= %f %f %f\n", str, rtk->xa[0], rtk->xa[1], rtk->xa[2]);
                }
