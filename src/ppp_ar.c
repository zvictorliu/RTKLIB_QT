                      ...
                      const nav_t *nav, const double *azel)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        { // in case the variance very small and reinitialize the MW
                        ...
                        double *P)
{
    ...
    for (i = 0; i < ns; i++)
    {
        ...
        if (conf_func(Nw, Bw, varw) < thres[1] || fabs(Nw - Bw) > thres[2])
            continue; // thres[1] =99.99% confidence to fix and the residuals smaller than 0.25.
        nw++;
        /*------added by xiang for output the errors debug only those pass fix of WL------*/
        // if (isfp_error == 0){//&& i == 0
        //	fp_error = fopen("C:\\GNSS_DATA\\tmpt\\WL_tempt_round.txt", "wt");
        //	isfp_error = 1;
        //	fprintf(fp_error, "%s\n", "week, tow(s), prn1, prn2, NIF_12, mw1(cycle), mw1_ssat(cycle), MW_orgl(cycle), mw2(cycle),MW2_orgl(cycle), SD_WL(cycle), var_WL(cycle), B1(cycle), var1(cycle) ");
        // }
        // else{
        //	fp_error = fopen("C:\\GNSS_DATA\\tmpt\\WL_tempt_round.txt", "at");
        // }
        // fprintf(fp_error, "%5d, %14.3f, %3d, %3d, %14.6f, %14.6f, %14.6f, %14.6f, %14.6f, %14.6f, %14.6f, %14.6f, %14.8f, %14.6f, %14.8f, %14.6f, %14.8f, %14.6f, %14.6f\n",
        //	week, tow, sat1[i], sat2[i], x[j], x[k], x[j] - x[k], amb1->LC[0] / lamW, rtk->ssat[sat1[i] - 1].mwmean / lamW, rtk->ssat[sat1[i] - 1].mw / lamW, amb2->LC[0] / lamW, rtk->ssat[sat2[i] - 1].mwmean / lamW, rtk->ssat[sat2[i] - 1].mw / lamW, Bw, varw, B1, var1, nav->wlbias[sat1[i] - 1], nav->wlbias[sat2[i] - 1]);

        ////matfprint(H, rtk->nx,nv, 10, 3, fp_error);
        // fclose(fp_error);
        ///*---------end of add the debug----------------------*/
...
static int fix_amb_ILS(rtk_t *rtk, const nav_t *nav, int *sat1, int *sat2, int *NW, int n)
{
    ...
    int i, j = 0, k = 0, m = 0, info, stat, flgs[MAXSAT] = {0}, max_flg = 0, l = 0;
    double *lam = {0}, var1;

    /*added by xiang inorder for outpur ambiguity 2015-11-16*/
    int week;
    double tow;
...
static void corr_phase_bias_fcb(obsd_t *obs, int n, const nav_t *nav, obsd_t *obs_tmpt)
{
    int i, j, k;
    double lam;
    ...
    for (j = 0; j < n; j++)
    {
        for (k = 0; k < NFREQ; k++)
        {
            ...
            lam = nav->lam[obs[j].sat - 1][k];

            if (lam == 0.0 ||
                obs[j].L[0] == 0.0 || obs[j].P[0] == 0.0 ||
                obs[j].L[k] == 0.0 || obs[j].P[k] == 0.0)
                continue;

            // printf("\n");
            // printf("FCB in ns: %lf\n", nav->fcb[i].bias[obs[j].sat - 1][k] / CLIGHT*1E9);
            // printf("CODEFCB in ns: %lf\n", nav->fcbcbias[obs[j].sat - 1][k] / CLIGHT*1E9);
            // printf("P1C1 in ns: %lf\n", nav->cbias[obs[j].sat - 1][1] / CLIGHT*1E9);
            // printf("P2C2 in ns: %lf\n", nav->cbias[obs[j].sat - 1][2] / CLIGHT*1E9);

            // obs[j].L[k] -= (nav->fcb[i].bias[obs[j].sat - 1][k] / lam);
            // obs[j].P[k] -= nav->fcbcbias[obs[j].sat - 1][k];
            // obs[j].L[k] -= (nav->fcbpbias[obs[j].sat - 1][k] / lam);
            // if (obs[j].code[k] == CODE_L1C) {
            //	obs[j].P[k] -= nav->fcbcbias[obs[j].sat - 1][0];
            // }
            ////else if (obs[j].code[k] == CODE_L1P ) {
            ////	obs[j].P[k] -= nav->fcbcbias[obs[j].sat - 1][1];
            ////}
            // else if (obs[j].code[k] == CODE_L2C || obs[j].code[k] == CODE_L2X ||
            //	obs[j].code[k] == CODE_L2L || obs[j].code[k] == CODE_L2S)
            //{
            //	obs[j].P[k] -= nav->fcbcbias[obs[j].sat - 1][2];
            /*}*/
            // else if (obs[j].code[k] == CODE_L2W){
            //	obs[j].P[k] -= nav->fcbcbias[obs[j].sat - 1][3];
            // }
            ...
            if (obs[j].code[k] == CODE_L1C)
            {
                obs[j].P[k] += nav->cbias[obs[j].sat - 1][1];
            }
            else if (obs[j].code[k] == CODE_L2C || obs[j].code[k] == CODE_L2X ||
                     obs[j].code[k] == CODE_L2L || obs[j].code[k] == CODE_L2S)
            {
                obs[j].P[k] += nav->cbias[obs[j].sat - 1][2];
#if 0
					obs[j].L[k] -= 0.25*lam[i]; /* 1/4 cycle-shift */
#endif
            }
        }
    }
                  ...
                  const nav_t *nav, const double *azel, double *x, double *P)
{
    ...
    /*if (nav->nf>=0) {
        corr_phase_bias_fcb(obs, n, nav, obs_tmpt);
    }*/
    ...
    {
        // return ppp_amb_IsLS(rtk, obs, n, exc, nav, azel, x, P);
        return 0;
    }
}
