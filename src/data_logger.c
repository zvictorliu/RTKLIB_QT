...
void log_obs(obsd_t *obs, int n)
{
   ...
   for (int i = 0; i < n; i++) {
      ...
      char *c1 = code2obs(obs[i].code[0],NULL), *c2 = code2obs(obs[i].code[1],NULL);
...
void log_satpos(obsd_t *obs, int nobs, gtime_t *time, double *rs, double *dts, double *var, int *svh)
{
   ...
   for (int i = 0; i < nobs; i++) {
      ...
      fprintf(satpos_log, "%s obs_time=%s t_time=%s rs=%13.3f %13.3f %13.3f rsv=%13.3f %13.3f %13.3f dts=%15.12f var=%10.7f svh=%02X\n",
               satid, obst_str, tt_str, rs[i*6], rs[1+i*6], rs[2+i*6], rs[3+i*6], rs[4+i*6], rs[5+i*6],
               dts[i*2], var[i], (int)svh[i]);
   }
