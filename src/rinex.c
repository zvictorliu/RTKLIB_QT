...
static int sat2code(int sat, char *code)
{
    ...
    case SYS_QZS:
        ...
        break;
    case SYS_BDS:
        sprintf(code, "C%2d", prn - MINPRNBDS + 1);
        break;
    case SYS_IRN:
...
static void convcode(double ver, int sys, const char *str, char *type)
{
    ...
    trace(4, "convcode: ver=%.2f sys=%2d type= %s -> %s\n", ver, sys, str, type);
}
/* decode obs header ---------------------------------------------------------*/
                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    { /* ver.3 */
        ...
        *tobs[i][nt] = '\0';
        ...
        /* change beidou B1 code: 3.02 （北斗的名字发生了改变）*/
                           ...
                           int *flag, int *sats)
{
    ...
    trace(5, "decode_obsepoch: ver=%.2f\n", ver);
    ...
    trace(5, "decode_obsepoch: time=%s flag=%d\n", time_str(*time, 3), *flag);
    return n;
}
/* decode obs data -----------------------------------------------------------*/
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    if (!obs->sat)
    {
        trace(5, "decode_obsdata: unsupported sat sat=%s\n", satid);
        stat = 0;
    }
    ...
    for (i = n = m = 0; i < ind->n; i++)
    {
        ...
        if (ind->type[i] == 0 && p[i] == 0) // 其它type怎么办？
            k[n++] = i; /* C1? index */
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (i = n = 0; *tobs[i]; i++, n++)
    {
        ...
        if (sys == SYS_BDS)
        {
            ...
            if (ind->frq[i] == 1)
                ind->frq[i] = 11; /* set C1P to a large value */
            else if (ind->frq[i] == 3)
                ind->frq[i] = 12; /* C5P to a large value */
            else if (ind->frq[i] == 2)
                ind->frq[i] = 1; /* 2I 2Q to B1I */
            else if (ind->frq[i] == 5)
                ind->frq[i] = 2; /* 7I to B2I */
            else if (ind->frq[i] == 4)
                ind->frq[i] = 3; /* 6I 6Q 6X 6A to B3I */
        }
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
    ...
    while ((n = readrnxobsb(fp, opt, ver, tsys, tobs, &flag, data, sta)) >= 0 && stat >= 0)
    {
        ...
        else
        {
            ...
            for (i = 0; i < n1; i++)
                obs->data[obs->n - i - 1].eventime = eventime;
            for (i = 0; i < n; i++)
                data[i].eventime = time0;
        }
                      ...
                      eph_t *eph)
{
    ...
    trace(5, "decode_eph: ver=%.2f sat=%2d\n", ver, sat);
                       ...
                       geph_t *geph)
{
    ...
    trace(5, "decode_geph: ver=%.2f sat=%2d\n", ver, sat);
                       ...
                       int *type, eph_t *eph, geph_t *geph, seph_t *seph)
{
    ...
    trace(5, "readrnxnavb: ver=%.2f sys=%d\n", ver, sys);
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    // trace(3, "RINEX VERSION=%.2f TYPE=%c SYS=%d\n", ver, *type, sys);
                       ...
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    trace(2, "readrnxfile: file=%s flag=%d index=%d\n", file, flag, index);
...
static void outobstype_ver3(FILE *fp, const rnxopt_t *opt)
{
    ...
    for (i = 0; navsys[i]; i++)
    {
        ...
        for (j = 0; j < opt->nobs[i]; j++)
        {
            ...
            if (navsys[i] == SYS_BDS)
            {
                if (opt->rnxver == 3.02 && tobs[1] == '2')
                    tobs[1] = '1';
                if (opt->rnxver != 3.02 && tobs[1] == '1')
                    tobs[1] = '2';
            }
            fprintf(fp, " %3s", tobs);
...
extern int outrnxnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    { /* ver.3 */
        ...
        if ((opt->navsys & SYS_BDS) && opt->rnxver >= 3.02)
        { /* ver.3.02 */
            if (opt->outtime)
            {
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    if (opt->rnxver > 2.99 || sys == SYS_GAL || sys == SYS_BDS)
    { /* ver.3 or ver.2 GAL */
