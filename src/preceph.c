...
extern int read_snxbias(const char *file, nav_t *nav)
{
    ...
    trace(1, "read_snxbias: file=%s\n", file);

    if (!(fp = fopen(file, "r")))
    {
        trace(1, "sinex bias parameters file open error: %s\n", file);
        return 0;
    }
    if (!fgets(buff, sizeof(buff), fp) || strstr(buff, "%=BIA") != buff)
    {
        trace(1, "sinex bias file read error: %s\n", file);
        fclose(fp);
        return 0;
    }
    while (fgets(buff, sizeof(buff), fp))
    {
        if (buff[0] == '-')
            sec = 0;
        else if (strstr(buff, "+BIAS/SOLUTION"))
        {
            sec = 1;
        }
        if (sec == 1)
        {
            ...
            if (sscanf(buff, "%*s %*s %s %s %lf:%lf:%lf %lf:%lf:%lf %*s %lf %lf", str1, str2,
                       &syear, &sdoy, &stod, &eyear, &edoy, &etod, &bias, &std) < 10)
                continue;
            ...
            if (!(sat = satid2no(str1)))
                continue;
            if (!(codeIdx = obs2code(str2 + 1, &frq)))
                continue;
                   ...
                   double *dts, double *vare, double *varc)
{
    ...
    {
        trace(4, "no prec ephem %s sat=%2d\n", time_str(time, 0), sat);
        return 0;
    }
    /* binary search */
    ...
    for (j = 0; j <= NMAX; j++)
    {
        ...
        {
            trace(4, "prec ephem outage %s sat=%2d\n", time_str(time, 0), sat);
            return 0;
        }
    }
    for (j = 0; j <= NMAX; j++)
                      ...
                      double *dant)
{
    ...
    if (NFREQ >= 3 && (satsys(sat, NULL) & (SYS_GAL | SYS_SBS | SYS_BDS)))
        k = 2;
