...
static void outsolstat(rtk_t *rtk, const nav_t *nav)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        {
            k = IB(i + 1, j, &rtk->opt);
            fprintf(fp_stat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%d,%d,%d,%d,%d,%d,%.2f,%.6f,%.5f,%.5f,%.4f,%.4f,%.4f,%.4f,%.4f,%d,%d,%d,%f,%f,%f,%f,%f\n",
                    ...
                    ssat->dtr, ssat->rs[0], ssat->rs[1], ssat->rs[2], ssat->dts[0] /*add dtr, rs, dts*/
                ); 
        }
    }
}
...
static double varerr(int sat, int sys, double el, double snr_rover, double snr_base,
                     double bl, double dt, int f, const prcopt_t *opt, const obsd_t *obs, double k_coff)
{
...
static double varerr1(int sat, int sys, double el, double bl, double dt, int f,
                      const prcopt_t *opt, double coff, const obsd_t *obs)
{
    trace(2, "rtkpos varerr1 coff=%f\n", coff);
    double c = opt->err[3] * bl / 1E4, d = CLIGHT * opt->sclkstab * dt, fact = 1.0;
    int nf = NF(opt);
    double cn0 = obs->SNR[f % nf] * 0.25;
    double ssi_tmp = cn0 / 5.0;
    double ssi = ssi_tmp < 9 ? ssi_tmp : 9.0;
    double a, b;
    double varr;
    ...
    fact *= opt->ionoopt == IONOOPT_IFLC ? 3.0 : 1.0;

    switch (lzw_config.varr_val)
    {
    case 1: // sine-elevation
        varr = SQR(a) + SQR(b / sin(el));
        break;
    case 2: // combined-v1
        varr = SQR((9 / ssi) * (a + b * exp(-el / 0.349)));
        break;
    case 3: // combined-v2
        varr = pow(10, -0.1 * cn0) / SQR(sin(el));
        break;
    default:
        varr = SQR(a) + SQR(b / sin(el));
        break;
    ...
    return 2.0 * coff * SQR(fact) * (varr + SQR(c)) + SQR(d);
}
    ...
    for (f = 0; f < nf; f++)
    {
        ...
        for (i = j = 0, offset = 0.0; i < ns; i++)
        {
            ...
            if (rtk->opt.ionoopt != IONOOPT_IFLC)
            {
                ...
                bias[i] = cp * lami - pr; // meter
                // bias[i] = cp - pr / lami; // cycle
            }
            else
            { /* use ionosphere free calc with 2 freqs */
                ...
                bias[i] = (C1 * lam1 * cp1 + C2 * lam2 * cp2) - (C1 * pr1 + C2 * pr2); // meter
                // trace(3, "  bias[i]=(%f * %f * %f + %f * %f * %f) - (%f * %f + %f * %f) = %f\n",
                //       C1, lam1, cp1, C2, lam2, cp2, C1, pr1, C2, pr2, bias[i]);
                // trace(3, "cp1=%f cp2=%f pr1=%f pr2=%f lam1=%f lam2=%f bias=%f\n", cp1, cp2, pr1, pr2, lam1, lam2, bias[i]);
            }
        ...
        for (i = 0; i < ns; i++)
        {
            ...
            // initx(rtk, (bias[i] - rtk->com_bias), SQR(rtk->opt.std[0]), IB(sat[i], f, &rtk->opt));
            // trace(3, "     sat=%3d: init phase=%.3f\n", sat[i], (bias[i] - rtk->com_bias) / lami);
            // trace(3, "   sat=%3d init phase x[%d]=%.3f\n", sat[i], IB(sat[i], f, &rtk->opt), (bias[i] - rtk->com_bias));
            rtk->ssat[sat[i] - 1].lock[f] = -rtk->opt.minlock;
        }
        free(bias);
    }
                      ...
                      const prcopt_t *opt, double *y)
{
    ...
    { /* iono-free linear combination */
        ...
        if (obs->L[0] != 0.0 && obs->L[1] != 0.0)
        {
            ...
            // trace(2, "y[0]= %f * %f * %f + %f * %f * %f - %f - %f = %f\n",
            //       C1, obs->L[0], lam[0], C2, obs->L[1], lam[1], r, dant_if, y[0]);
        }
    ...
    else
    {
        for (i = 0; i < nf; i++)
        {
            ...
            if (obs->P[i] != 0.0){
                ...
                // if (base == 1){
                //     trace(2, "y[%d]= %.4f - %.4f - %.4f = %.4f\n", i + nf, obs->P[i], r, dant[i], y[i + nf]);
                // }
            }
...
/* undifferenced phase/code residuals ----------------------------------------
 ...
 args:  I   base:  0=base,1=rover 似乎是错了
                 ...
                 int index, double *y, double *e, double *azel)
{
    ...
    trace(2, "zdres   : n=%d rr=%.3f %.3f %.3f base=%d\n", n, rr[0], rr[1], rr[2], base);
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* excluded satellite? */
        if (satexclude(obs[i].sat, var[i], svh[i], opt, obs[i].time))
            continue;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        trace(4, "sat=%2d %s %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n",
              obs[i].sat, id, rs[i * 6], rs[1 + i * 6], rs[2 + i * 6], dts[i * 2], azel[i * 2] * R2D,
              azel[1 + i * 2] * R2D);
    }
    trace(4, "y=\n");
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    int week, gpst;
    gpst = (int)time2gpst(obs[0].time, &week);
    ...
    for (m = 0; m < 5; m++)
    {
        ...
        for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++)
        {
            ...
            if (i < 0){
                // trace(3, "no ref sat sys=%d code=%d\n", m, code);
                continue;
            }
            
            trace(4, "ref sat: %d sys=%d code=%d\n", sat[i], m, code);
            ...
            for (j = 0; j < ns; j++)
            {
                ...
                trace(4, "before rec, v[nv]=(%f - %f) - (%f - %f)=%f\n", y[f + iu[i] * nf * 2], y[f + ir[i] * nf * 2], y[f + iu[j] * nf * 2], y[f + ir[j] * nf * 2], v[nv]);
                ...
                if (!code)
                {
                    ...
                    else
                    {
                        ...
                        // trace(3, "IFLC: v[nv] -= (%f(x[%d]) - %f(x[%d])) => %f\n", 
                        // x[IB(sat[i], frq, opt)], IB(sat[i], frq, opt), x[IB(sat[j], frq, opt)], IB(sat[j], frq, opt), v[nv]);
                        if (H)
                        {
                ...
                if (lzw_config.varr_val == 0){
                    ...
                    satno2id(sat[i], id);
                    double coff_val = lzw_config.nlos_val >= 2 ? get_coff(id, week, gpst) : 1;
                    Ri[nv] = varerr1(sat[i], sysi, azel[1 + iu[i] * 2], bl, dt, f, opt, coff_val, obs + iu[i]);
                    

                    satno2id(sat[j], id);
                    coff_val = lzw_config.nlos_val >= 2 ? get_coff(id, week, gpst) : 1;
                    Rj[nv] = varerr1(sat[j], sysj, azel[1 + iu[j] * 2], bl, dt, f, opt, coff_val, obs + iu[j]);
                ...
                satno2id(sat[i], idi);
                satno2id(sat[j], idj);
                trace(4, "ddres sat=%s-%s(%d-%d) %s%d v=%13.3f R=%9.6f %9.6f icb=%9.3f lock=%5d x=%9.3f\n", idi, idj, sat[i],
        ...
        for (f = 0, k = na; f < nf; f++, k += MAXSAT)
        {
            ...
            for (i = k; i < k + MAXSAT; i++)
            {
                ...
                if (rtk->ssat[i - k].lock[f] >= 0 && !(rtk->ssat[i - k].slip[f] & 2) &&
                    rtk->ssat[i - k].azel[1] >= rtk->opt.elmaskar && !nofix && !rtk->ssat[i - k].nlos)
                {
            ...
            for (j = k; j < k + MAXSAT; j++)
            {
                ...
                if (rtk->ssat[j - k].lock[f] >= 0 && !(rtk->ssat[j - k].slip[f] & 2) &&
                    rtk->ssat[i - k].vsat[f] &&
                    rtk->ssat[j - k].azel[1] >= rtk->opt.elmaskar && !nofix && !rtk->ssat[j - k].nlos)
                {
                  ...
                  const nav_t *nav)
{
    ...
    // {
    //     // int snr = (int)obs[ii].SNR[0];
    //     trace(2, "obs[i].SNR[0] = %d\n", (int)obs[ii].SNR[0] / 4);
    // }
    ...
    trace(3, "x(0)=");
    tracemat(3, rtk->x, 1, NR(opt), 13, 4);
    ...
    if (stat != SOLQ_NONE && zdres(0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel))
    {
        ...
        if (valpos(rtk, v, R, vflg, nv, 4.0))
        {
            ...
            if (rtk->sol.ns < 4){
                ...
                trace(2, "float sol: lack of valid satellites ns = %d\n", rtk->sol.ns);
                stat = SOLQ_NONE;
            }
        }
        else{
            trace(2, "float sol: validation failed\n");
            stat = SOLQ_NONE;
        }
    }
    ...
    if (stat != SOLQ_NONE){
        ...
        trace(2, "float rr= %.3f %.3f %.3f\n", rtk->x[0], rtk->x[1], rtk->x[2]);

        char msg[16];
        sprintf(msg, "%2d", rtk->sol.ns);
        store_info(obs[0].time, "ns_float", msg);

        // 计算float解的DOP
        double dop[4];
        int ns_v = 0;
        double azel_v[2 * MAXOBS];
        for (i = 0; i < nu; i++){
            if (rtk->ssat[obs[i].sat - 1].vsat[0]){
                azel_v[2 * ns_v] = azel[2 * i];
                azel_v[2 * ns_v + 1] = azel[2 * i + 1];
                ns_v++;
            }
        }
        
        trace(4, "float azel_v: ns_v = %d\n", ns_v);
        tracemat(4, azel_v, 2, ns_v, 2, 4);

        dops(ns_v, azel_v, rtk->opt.elmin, dop);

        // 存储float解的DOP
        char dop_msg[16];
        sprintf(dop_msg, "%.3f", dop[1]);
        store_info(obs[0].time, "dop_float", dop_msg);
    ...
    else if (stat != SOLQ_NONE)
    {
        ...
        if (manage_amb_LAMBDA(rtk, bias, xa, sat, nf, ns) > 1)
        {
         ...
         trace(3, "manage_amb_LAMBDA failed\n");
    ...
    if (stat == SOLQ_FIX)
    {
        ...
        trace(2, "fix azel_v: ns_v = %d\n", ns_v);
        tracemat(2, azel_v, 2, ns_v, 2, 4);
        ...
        char dop_msg[16];
        sprintf(dop_msg, "%.3f", dop[1]);
        store_info(obs[0].time, "dop_fix", dop_msg);
        sprintf(dop_msg, "%.3f", dop[2]);
        store_info(obs[0].time, "dop_fix", dop_msg);
    ...
    for (i = 0; i < n; i++){
        // add by lzw
        /* azel to ssat*/
        rtk->ssat[obs[i].sat - 1].azel[0] = azel[2 * i];
        rtk->ssat[obs[i].sat - 1].azel[1] = azel[2 * i + 1];
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    // for (i = 0; i < MAXSAT; i++){
    //     satno2id(i + 1, id);
    //     if (lzw_config.ar_val == 1){
    //         rtk->ssat[i].nlos = is_NLOS(id, week, gpst);
    //     }
    //     else{
    //         rtk->ssat[i].nlos = 0;
    //     }
    ...
    if (opt->mode >= PMODE_PPP_KINEMA)
    {
        ...
        // trace(3, "PPP: sum of P = %f\n", rtk->P[0] + rtk->P[1] + rtk->P[2]);
        // if ((rtk->P[0] + rtk->P[1] + rtk->P[2]) < 30 * 30) // 协方差之和有要求？
        //     outsolstat(rtk, nav);
        // else
        //     return 0;
        outsolstat(rtk, nav);
