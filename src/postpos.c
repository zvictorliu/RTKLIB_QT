...
static void corr_phase_bias_ssr(obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < NFREQ; j++)
        {
            ...
            // trace(2, "corr_phase_bias_ssr sat=%2d code=%2d L=%13.3f delta=%f\n", obs[i].sat, code, obs[i].L[j], nav->ssr[obs[i].sat-1].pbias[code-1]/lam);
        }
}
/* process positioning -------------------------------------------------------*/
                        ...
                        nav_t *nav, sbs_t *sbs, lex_t *lex)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (strstr(infile[i], "%r") || strstr(infile[i], "%b"))
            continue;
        if ((ext = strrchr(infile[i], '.')) &&
            (strlen(ext) == 4 && (ext[3] == 'i' || ext[3] == 'I' || !strcmp(ext, ".INX")))) // 读的后缀 .INX的不行
        {
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    {
        if (strlen(ext) == 4 && (ext[3] == 'i' || ext[3] == 'I' || !strcmp(ext, ".INX"))) // 读的后缀 .INX的不行
        {
